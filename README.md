...
Проект имеет **более 80 unit-тестов**, покрывающих ключевую бизнес-логику.

### 📊 Покрытие тестами

| Компонент | Покрытие | Ключевые тесты |
|-----------|----------|-----------------|
| **Application Layer** | **92%** | ✅ Калькуляторы цены<br>✅ Сервис покупок<br>✅ Валидация DTO<br>✅ Маппинги |
| **Domain Layer** | **100%** | ✅ Сущности Skin и Purchase |
| **Infrastructure Layer** | **80%+** | ✅ BtcRateService (кэш, fallback)<br>✅ Конфигурации EF<br>✅ Обработка ошибок API |
| **WebAPI Layer** | **70%+** | ✅ ExceptionHandlingMiddleware<br>✅ Health Checks |

### ✅ Что тестируется

#### Application Layer (92% покрытия)
- **IPriceCalculator** — проверка формул Standard и Promo с различными входными данными
- **PurchaseService** — создание покупки, идемпотентность, optimistic concurrency
- **SkinService** — CRUD операции, soft delete, фильтрация и пагинация
- **FluentValidation** — все DTO проходят валидацию (пустые поля, отрицательные цены и т.д.)
- **AutoMapper** — корректность маппингов

#### Infrastructure Layer
- **BtcRateService** — кэширование, fallback при ошибках API, обработка таймаутов
- **AppDbContext** — конфигурации сущностей, глобальные фильтры
- **Миграции** — проверка создания схемы БД

#### WebAPI Layer
- **ExceptionHandlingMiddleware** — обработка всех типов исключений (400, 404, 409, 503)
- **HealthChecks** — проверка доступности БД и внешнего API
...
- [ ] **Redis** — распределённое кэширование курса
- [ ] **JWT аутентификация** — вместо мок-авторизации
- [ ] **Интеграционные тесты** — с TestContainers
- [ ] **Метрики** — Prometheus + Grafana
- [ ] **CI/CD** — GitHub Actions для автоматического тестирования
- [ ] **SignalR** — уведомления об изменении цены
...
- **CoinGecko** — за бесплатное и надёжное API курсов криптовалют
- **Microsoft** — за отличный .NET 8 и ASP.NET Core
- **Open source сообщество** — за инструменты, делающие разработку лучше
